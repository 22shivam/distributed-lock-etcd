...
class DistributedLock:
    ...
    def ttl_remaining(self) -> int:
        """
        Returns the number of seconds left on our lease, or -1 if unknown.
        """
        try:
            # etcd3 client method that returns (TTL, granted_ttl)
            ttl, _ = client.time_to_live(self.lease.id, keys=False)
            return ttl
        except Exception:
            return -1
