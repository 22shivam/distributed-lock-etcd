...
import etcd3, uuid, time, threading
...
def _wait_for_deletion(key: str, wait_time: float) -> bool:
    ...
    Returns True if we saw a deletion, False on timeout.
    """
    events, cancel = client.watch(key)
    # schedule a cancel() after wait_time
    ...
    # if we exit the loop, either cancel() fired or stream ended
    timer.cancel()
    return False
...
if __name__ == "__main__":
    ...
    # 2) Lease expiry test
    owner1, lease1 = acquire_lock("expiry_demo", ttl=10, timeout=2)
