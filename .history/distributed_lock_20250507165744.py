...
class DistributedLock:
    ...
    Usage:
        try:
            with DistributedLock("resource_name", ttl=30, timeout=10) as lock:
            ...
            handle_failure()
    """
    def __init__(self, resource: str, ttl: int = 30, timeout: float = None):
    ...
    def __enter__(self):
        ...
        try:
            ...
            print(f"Error acquiring lock for {self.resource}: {e}")
            raise
        return self

    def __exit__(self, exc_type, exc_val, exc_tb):
        # Always attempt release if we acquired
        try:
...
if __name__ == "__main__":
    ...
    with DistributedLock("expiry_demo", ttl=3, timeout=5) as lock1:
        print("Lock held by", lock1.owner, "for ~3s (no refresh)...")
        time.sleep(2)
    with DistributedLock("expiry_demo", ttl=10, timeout=5) as lock2:
